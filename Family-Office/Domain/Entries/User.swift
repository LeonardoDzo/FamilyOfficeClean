...
public struct User: Decodable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        if  let values = try? decoder.container(keyedBy: CodingKeys.self) {
            ...
            birth = try values.decodeIfPresent(Int.self, forKey: .birth) ?? 0
            ...
            curp = values.decodeSafelyIfPresent(String.self, forKey: .curp) ?? ""
            nss = values.decodeSafelyIfPresent(String.self, forKey: .nss) ?? ""
            rfc = values.decodeSafelyIfPresent(String.self, forKey: .rfc) ?? ""
            bloodyType = values.decodeSafelyIfPresent(String.self, forKey: .bloodyType) ?? ""
